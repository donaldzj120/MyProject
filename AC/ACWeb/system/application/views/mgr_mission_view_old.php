   ...
   <?php if(!empty($result_td)) {?>
    ...
    <table id="pagedtable" border="0" cellpadding="3" cellspacing="1" bgcolor="#999999" width=100%>
	  ...
	  <?php echo form_open('mgr_mission/dtb_confirm',array('name' => 'dtb'.$td['id'])); ?>
        ...
        <td width="5%"><?php if($td['mission_status'] > 0) echo $td['zhixing_id']; else echo form_dropdown($td['id'], $dl, '-');?></td>
        <td width="6%"><?php if($td['del_status'] == 1)echo $statusText[3];else echo $statusText[$td['mission_status']];?></td>
        <!-- <td width="6%"><?php if($td['del_status'] == 1) echo '取消中';?></td> -->
        <td width="13%"><?php echo $td['recv_date'];?></td>
        <td width="13%"><?php echo $td['fenpei_date'];?></td>
		...
		?></td>
        <td width="5%"><?php if(!empty($td['fileover']))echo anchor($upurl.$td['fileover'],'下載', array('target' => '_blank'));?></td>
        <td width="5%"><?php if(!empty($td['siryo']))echo anchor($siryo.$td['siryo'],'下載', array('target' => '_blank'));?></td>
		<td width="5%"><?php 
		if($td['del_status'] == 1){ 
      	...
      	<td width="6%"><a href="javascript:document.<?php echo 'dtb'.$td['id']; ?>.submit();">配送確認</a></td>
	  <?php echo form_close(); ?>
	</tr>
      <?php } ?>
      </tbody>
