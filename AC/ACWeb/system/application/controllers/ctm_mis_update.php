...
class Ctm_mis_update extends Controller {
	...
	function index()
	{
		...
		if ($this->uri->segment(2) != FALSE){
			...
			if(isset($data['query']['ka_tel']) && !empty($data['query']['ka_tel']))
	      	{
	      		$katel = explode('-',$data['query']['ka_tel']);
			...
			if(isset($data['query']['todoke_tel']) && !empty($data['query']['todoke_tel']))
	      	{
	      		$katel = explode('-',$data['query']['todoke_tel']);
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$this->load->view('ctm_mis_update_view',$data);
