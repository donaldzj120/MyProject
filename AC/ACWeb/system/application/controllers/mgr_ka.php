<?php
class Mgr_ka extends Controller {
	function index()
	{
		...
		echo form_open('mgr_ka/ka_search')."\n";
		echo '<table align="right" border="0" cellpadding="0" cellspacing="0" bgcolor="#9999999" width=60%>'."\n";
		echo '<tr><th bgcolor="#E0EFFF" width="15%">'.$th['ka_name'].'</th>'."\n";
		echo '<td width="30%" align="center" bgcolor="#FFFFFF">'."\n";
		$ka_data = array(
              'name'        => 'ka_name',
              'id'          => 'ka_name',
              'maxlength'   => '50',
		...
		$ka_data = array(
              'type'        => 'image',
              'src'         => $this->config->item('base_url').'/images/button_kensaku.gif',
            );
		...
		echo '<tr>'."\n";
		echo '<th bgcolor="#E0EFFF" width="30%">'.$th['ka_name'].'</th>'."\n";
		echo '<th bgcolor="#E0EFFF" width="16%">'.$th['ka_tel'].'</th>'."\n";
		echo '<th bgcolor="#E0EFFF" width="16%">'.$th['ka_post'].'</th>'."\n";
		echo '<th bgcolor="#E0EFFF" width="30%">'.$th['ka_add'].'</th>'."\n";
		echo '<th bgcolor="#E0EFFF" width="8%">'.$th['ka_kakutei'].'</th>'."\n";
		echo '</tr>'."\n";
		echo '</table>';
	...
	function ka_search()
	{
		...
		if(isset($_POST['ka_name']) && !empty($_POST['ka_name']))
		{
			$sql = '';
			$sql = $sql."select * ";
			$sql = $sql."from ka ";
			$sql = $sql."where ka_name1 like '%".$_POST['ka_name']."%' or ka_name2 like '%".$_POST['ka_name']."%' ";
