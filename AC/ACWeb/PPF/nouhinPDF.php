...
try {
	...
	// create PDF
	$pdf = Zend_Pdf::load('../PDFSample/nouhinsho.pdf');
	$page = $pdf->pages[0];
	...
	IF($row = mysql_fetch_array($sagyou)){
		...
		IF($row['ka_add2'] != null)
		{
			$page->drawText($row['ka_add2'], 138, 640, 'UTF-8');
		}
		IF($row['ka_add3'] != null)
		{
		...
		{
			$page->drawText($row['todoke_add2'], 138, 450, 'UTF-8');
		}
	...
	$pdf->save('../pdf/nouhinsho.pdf');
	header('Content-type: application/pdf');
	//header('filename=pdf');
	readfile('../pdf/nouhinsho.pdf');
